...
const FileCard = ({ title, subtitle, status, date, onView, isLead }: any) => (
  <div className="bg-white p-6 rounded-[32px] hover:shadow-xl hover:shadow-[#17372C]/20 transition-all group relative overflow-hidden shadow-sm">
    {isLead !== undefined && (
      <div className={`absolute top-0 right-0 px-3 py-1 text-xs font-semibold ${isLead ? 'bg-[#27664D] text-white' : 'bg-[#F2F1E8] text-[#27664D]'}`}>
        {isLead ? 'Lead' : 'Assisting'}
      </div>
    )}
    <div className="flex justify-between items-start mb-4">
      <div className="space-y-1 pr-12">
        <h4 className="text-sm font-semibold text-[#17372C] group-hover:text-[#27664D] transition">{title}</h4>
        <p className="text-xs font-medium text-[#8AA79B] uppercase tracking-wide">{subtitle}</p>
      </div>
      <span className={`px-2.5 py-1 rounded-lg text-xs font-semibold ${status === 'Completed' ? 'bg-emerald-50 text-emerald-600' : 'bg-[#FBFAF6] text-[#27664D]'}`}>
        {status}
      </span>
    </div>
    <div className="flex justify-between items-center mt-6">
      <p className="text-xs font-medium text-[#8AA79B]">{date}</p>
      <button onClick={onView} className="bg-[#17372C] text-white px-4 py-2 rounded-xl text-xs font-semibold uppercase tracking-wider hover:bg-[#27664D] transition">
        View File
      </button>
    </div>
  </div>
...
export default function LawyerDashboard() {
  ...
  const inp = "w-full bg-[#FFFFFF]/50 border border-[#DDE5DD] p-4 rounded-2xl text-sm text-[#17372C] outline-none focus:ring-4 focus:ring-[#D4B65D]/10 focus:border-[#D4B65D] transition shadow-sm";
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-center mb-10">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => navigate("/requisitions")} className="bg-white text-[#17372C] hover:bg-[#FBFAF6] px-5 py-3.5 rounded-2xl transition text-xs font-semibold uppercase tracking-wider shadow-lg hidden md:block">
                📝 Requisitions
              </button>
              <button onClick={() => setIsTaskModalOpen(true)} className="bg-[#27664D] hover:bg-[#D4B65D] text-white px-5 py-3.5 rounded-2xl transition text-xs font-semibold uppercase tracking-wider shadow-lg hidden md:block">
                + Assign Clerk Task
              </button>
              {/* Mobile actions menu button or icons could go here if space is tight, but we'll show simplified buttons on mobile */}
              <div className="md:hidden flex gap-2">
                 <button onClick={() => navigate("/requisitions")} className="bg-white text-[#17372C] p-3.5 rounded-2xl shadow-lg">📝</button>
                 <button onClick={() => setIsTaskModalOpen(true)} className="bg-[#27664D] text-white p-3.5 rounded-2xl shadow-lg">➕</button>
              </div>
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <div onClick={() => myData.nextHearing && navigate(`/lawyer/cases/${myData.nextHearing.id}`)}
              className={`p-6 rounded-[28px] shadow-[0_4px_12px_rgba(0,0,0,0.05)] transition cursor-pointer ${myData.nextHearing ? "bg-[#27664D] shadow-lg hover:bg-[#D4B65D]" : "bg-white/5"}`}>
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-12 space-y-8">
            ...
            <div className="bg-white/10 backdrop-blur-2xl border border-white/20 rounded-[48px] p-10 md:p-14 text-white shadow-[0_32px_64px_-16px_rgba(0,0,0,0.5)] max-w-2xl w-full text-center relative overflow-hidden group">
              ...
              <div className="relative z-10">
                ...
                <p className="text-blue-100/80 text-lg leading-relaxed mb-10 font-medium max-w-lg mx-auto">
                  ...
                  Real-time status updates and court registry reference tracking are now at your fingertips.
                </p>
        ...
        {myData.pendingDeadlines.length > 0 && (
          <div className="bg-white p-8 rounded-[40px] shadow-sm border-none">
            <h2 className="text-sm font-semibold text-[#17372C] mb-6 uppercase tracking-wider">Upcoming Court Deadlines</h2>
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="text-xs font-semibold text-[#8AA79B] uppercase tracking-wider border-b border-[#F2F1E8]">
                ...
                <tbody className="text-sm">
                  {myData.pendingDeadlines.map(deadline => (
                    <tr key={deadline.id} className="border-b border-slate-50 last:border-0 hover:bg-[#FFFFFF]/50 transition">
                      <td className="py-4 pr-4">
                        <p className="font-semibold text-[#17372C]">{deadline.title}</p>
                      </td>
                      <td className="py-4 pr-4">
                        <button onClick={() => navigate(`/lawyer/cases/${deadline.caseId}`)} className="text-[#D4B65D] hover:text-[#27664D] font-medium text-xs uppercase tracking-wide transition truncate max-w-[200px] block text-left">
                          ⚖️ {deadline.caseFileName}
                        </button>
                      </td>
                      <td className="py-4 pr-4 font-medium whitespace-nowrap">
                        <div className="flex flex-col">
                          <span className="text-[#27664D]">{new Date(deadline.dueDate).toLocaleDateString()}</span>
                      ...
                      <td className="py-4 pr-4">
                        <span className="px-2.5 py-1 rounded text-[10px] font-semibold bg-[#FBFAF6] text-[#27664D] border border-[#DDE5DD] uppercase">
        ...
        {/* TASKS TABLE */}
        <div className="bg-white p-8 rounded-[40px] shadow-sm border-none">
          <h2 className="text-sm font-semibold text-[#17372C] mb-6 uppercase tracking-wider">Instructions to Clerks</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="text-xs font-semibold text-[#8AA79B] uppercase tracking-wider border-b border-[#F2F1E8]">
              ...
              <tbody className="text-sm">
                {myTasks.length > 0 ? myTasks.map(task => (
                  <tr key={task.id} className="border-b border-slate-50 last:border-0 hover:bg-[#FFFFFF]/50 transition">
                    <td className="py-4">
                      <p className="font-semibold text-[#17372C]">{task.title}</p>
                      <p className="text-xs text-[#8AA79B] mt-0.5">{task.description}</p>
                      {task.relatedFileName && <p className="text-xs text-[#D4B65D] font-medium mt-1 uppercase tracking-wide">📎 {task.relatedFileName}</p>}
                    </td>
                    <td className="py-4 font-medium text-[#27664D]">{task.assignedToName}</td>
                    ...
                    <td className="py-4 italic text-[#8AA79B] text-xs">{task.clerkNote || "Awaiting update…"}</td>
                    <td className="py-4 text-right space-x-4">
                      <button onClick={() => openEditModal(task)} className="text-[#27664D] hover:text-blue-800 font-semibold text-xs">Edit</button>
                      <button onClick={() => deleteTask(task.id)} className="text-red-400 hover:text-red-600 font-semibold text-xs">Delete</button>
                    </td>
                  </tr>
                )) : (
                  <tr><td colSpan={5} className="py-8 text-center text-sm italic text-[#8AA79B]">No instructions currently pending.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div>
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            <div className="flex gap-8 border-b border-[#DDE5DD] overflow-x-auto">
              {(["Cases", "Transactions", "Letters", "Drafts", "Registry", "Calendar"] as const).map(tab => (
                <button key={tab} onClick={() => { setActiveTab(tab); setSearchQuery(""); }}
                  className={`pb-3 text-xs font-semibold uppercase tracking-wider transition border-b-2 -mb-px flex items-center gap-2 whitespace-nowrap ${activeTab === tab ? "border-[#17372C] text-[#17372C]" : "border-transparent text-[#8AA79B] hover:text-[#27664D]"}`}>
            ...
            <div className="relative">
              <input type="text" placeholder={`Search ${activeTab.toLowerCase()}…`} value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                className="pl-10 pr-10 py-3 rounded-2xl border border-[#DDE5DD] bg-white shadow-sm text-sm outline-none focus:ring-2 focus:ring-[#D4B65D] w-72" />
              <span className="absolute left-3.5 top-3.5 text-[#8AA79B] text-sm">🔍</span>
              {searchQuery && <button onClick={() => setSearchQuery("")} className="absolute right-3.5 top-3 text-[#8AA79B] hover:text-[#27664D] text-lg leading-none">×</button>}
            </div>
          </div>

          {searchQuery && <p className="text-xs font-medium text-[#8AA79B] mb-4">{currentCount} result{currentCount !== 1 ? "s" : ""} for "{searchQuery}"</p>}
          ...
          <div className={activeTab === "Calendar" ? "hidden" : "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"}>
            {activeTab === "Cases" && (filteredCases.length > 0
              ...
              : <p className="col-span-3 text-center text-sm italic text-[#8AA79B] py-10">No cases found{searchQuery ? ` for "${searchQuery}"` : ""}.</p>
            )}
            {activeTab === "Transactions" && (filteredTxs.length > 0
              ...
              : <p className="col-span-3 text-center text-sm italic text-[#8AA79B] py-10">No transactions found{searchQuery ? ` for "${searchQuery}"` : ""}.</p>
            )}
            {activeTab === "Letters" && (filteredLtrs.length > 0
              ...
              : <p className="col-span-3 text-center text-sm italic text-[#8AA79B] py-10">No letters found{searchQuery ? ` for "${searchQuery}"` : ""}.</p>
            )}
            {activeTab === "Drafts" && (
              <div className="col-span-3 space-y-8">
                <div>
                  <div className="flex justify-between items-center mb-4">
                    <p className="text-xs font-semibold text-[#8AA79B] uppercase tracking-wider">Drafts assigned to me ({filteredAssignedDrafts.filter(d => d.status === draftsTab).length})</p>
                    <div className="flex bg-[#FBFAF6] p-1 rounded-xl">
                      <button
                        onClick={() => setDraftsTab("Pending")}
                        className={`px-4 py-1.5 rounded-lg text-xs font-bold uppercase transition ${draftsTab === "Pending" ? "bg-white shadow-sm text-[#17372C]" : "text-[#8AA79B] hover:text-[#27664D]"}`}
                      >
                        Pending ({filteredAssignedDrafts.filter(d => d.status === 'Pending').length})
                      </button>
                      <button
                        onClick={() => setDraftsTab("Completed")}
                        className={`px-4 py-1.5 rounded-lg text-xs font-bold uppercase transition ${draftsTab === "Completed" ? "bg-white shadow-sm text-[#17372C]" : "text-[#8AA79B] hover:text-[#27664D]"}`}
                      >
                    ...
                    <table className="w-full text-left">
                      ...
                      <tbody className="text-sm">
                        {filteredAssignedDrafts.filter(d => d.status === draftsTab).length > 0 ? filteredAssignedDrafts.filter(d => d.status === draftsTab).map(draft => (
                          <tr key={draft.id} className="border-b border-slate-50 last:border-0 hover:bg-[#FFFFFF]/50 transition">
                            <td className="py-4 pr-4 align-top">
                              <p className="font-semibold text-[#17372C]">{draft.title}</p>
                              <p className="text-xs text-[#8AA79B] mt-1 max-w-sm">{draft.description}</p>
                            </td>
                            <td className="py-4 pr-4 align-top">
                              <button onClick={() => navigate(`/lawyer/cases/${draft.caseId}`)} className="text-[#D4B65D] hover:text-[#27664D] font-medium text-xs uppercase tracking-wide transition truncate max-w-[200px] block text-left">
                                ⚖️ {draft.caseFileName}
                              </button>
                            </td>
                            <td className="py-4 pr-4 align-top">
                              <span className="text-[#27664D] font-medium">{draft.requestedByName}</span>
                            </td>
                            <td className="py-4 pr-4 align-top">
                              <div className="flex flex-col gap-2 items-start">
                                ...
                                <span className="text-xs text-[#8AA79B] font-medium">Due {draft.deadline}</span>
                              </div>
                            </td>
                          ...
                          <tr><td colSpan={5} className="py-8 text-center text-sm italic text-[#8AA79B]">No {draftsTab.toLowerCase()} drafts found.</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
                ...
                {filteredRequestedDrafts.length > 0 && (
                  <div>
                    <p className="text-xs font-semibold text-[#8AA79B] uppercase tracking-wider mb-4">Drafts I've delegated ({filteredRequestedDrafts.length})</p>
                    <div className="bg-white p-8 rounded-[40px] shadow-sm border-none overflow-x-auto">
                      <table className="w-full text-left">
                        <thead>
                          <tr className="text-xs font-semibold text-[#8AA79B] uppercase tracking-wider border-b border-[#F2F1E8]">
                        ...
                        <tbody className="text-sm">
                          {filteredRequestedDrafts.map(draft => (
                            <tr key={draft.id} className="border-b border-slate-50 last:border-0 hover:bg-[#FFFFFF]/50 transition">
                              ...
                              <td className="py-4 pr-4 align-top">
                                <span className="text-[#27664D] font-medium">{draft.assignedToName}</span>
                              </td>
                              <td className="py-4 align-top">
                                <div className="flex flex-col gap-2 items-start">
                                  <span className={`px-2.5 py-1 rounded text-[10px] font-black uppercase ${draft.status === 'Completed' ? 'bg-emerald-50 text-emerald-600' : 'bg-[#FBFAF6] text-[#27664D]'}`}>
                                    {draft.status}
                                  </span>
                                  <span className="text-xs text-[#8AA79B] font-medium">Due {draft.deadline}</span>
                                </div>
            ...
            {activeTab === "Registry" && (
              <div className="col-span-3 space-y-8">
                ...
                <div>
                  <p className="text-xs font-semibold text-[#8AA79B] uppercase tracking-wider mb-4">My Outgoing Filing Requests ({filteredRequestedFilings.length})</p>
                      ...
                      <div key={filing.id} className="bg-white p-6 rounded-[32px] shadow-sm border-none">
                        <div className="flex justify-between items-start mb-3">
                          <div className="space-y-1 pr-4">
                            <h4 className="text-sm font-semibold text-[#17372C]">{filing.documentName}</h4>
                            <p className="text-[10px] font-bold text-[#D4B65D] uppercase">⚖️ {filing.caseFileName}</p>
                          </div>
                          <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase ${filing.status === 'Completed' ? 'bg-emerald-50 text-emerald-600' : 'bg-[#27664D] text-white animate-pulse'}`}>{filing.status}</span>
                        </div>
                        <p className="text-xs text-[#8AA79B] mb-4 line-clamp-2">{filing.description || "No specific instructions provided."}</p>
                        <div className="flex flex-wrap gap-x-4 gap-y-2 text-[10px] font-black text-[#8AA79B] uppercase">
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-50">
                          <button onClick={() => navigate(`/lawyer/cases/${filing.caseId}`)} className="text-[10px] font-black text-[#27664D] uppercase hover:underline">View Matter Details</button>
                        </div>
                      </div>
                    ))}
                    {filteredRequestedFilings.length === 0 && (
                      <p className="col-span-3 text-center text-sm italic text-[#8AA79B] py-10">No outgoing filings found.</p>
                    )}
                  </div>
                </div>
                ...
                {filteredAssignedFilings.length > 0 && (
                  <div>
                    <p className="text-xs font-semibold text-[#8AA79B] uppercase tracking-wider mb-4">Registry Filings Assigned to Me ({filteredAssignedFilings.length})</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div key={filing.id} className={`bg-white p-6 rounded-[32px] transition-all shadow-md`}>
                          <div className="flex justify-between items-start mb-3">
                            <div className="space-y-1">
                              <h4 className="text-sm font-semibold text-[#17372C]">{filing.documentName}</h4>
                              <p className="text-[10px] font-bold text-[#8AA79B] uppercase">⚖️ {filing.caseFileName}</p>
                            </div>
                            <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase ${filing.status === 'Completed' ? 'bg-emerald-50 text-emerald-600' : 'bg-[#FBFAF6] text-[#27664D]'}`}>{filing.status}</span>
                          </div>
                          <div className="flex flex-wrap gap-4 text-[10px] font-black text-[#8AA79B] uppercase mb-4">
                          ...
                          <div className="space-y-2">
                            ...
                            <button onClick={() => navigate(`/lawyer/cases/${filing.caseId}`)} className="w-full bg-[#17372C] text-white py-2.5 rounded-xl text-xs font-semibold uppercase tracking-wider hover:bg-[#27664D] transition">
                              Open Matter Details
                            </button>
                          </div>
                        </div>
      ...
      {isTaskModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative bg-white w-full max-w-4xl rounded-[32px] shadow-2xl overflow-hidden flex flex-col md:flex-row">
            <div className="bg-gradient-to-br from-[#17372C] to-[#27664D] md:w-2/5 p-10 text-white flex-col justify-between hidden md:flex">
              <div>
            ...
            <div className="bg-white md:w-3/5 p-8 md:p-10 flex flex-col max-h-[90vh] overflow-y-auto w-full">
              <div className="flex justify-between items-center mb-8 md:hidden">
                <h3 className="text-xl font-bold text-[#17372C]">{editingTaskId ? "Edit Task" : "Assign Clerk"}</h3>
                <button onClick={closeModal} className="w-8 h-8 flex items-center justify-center rounded-full bg-[#FBFAF6] text-[#8AA79B] hover:bg-[#F2F1E8] transition">✕</button>
              </div>

              <div className="flex-1 space-y-5">
                <div>
                  <label className="text-xs font-semibold text-[#8AA79B] uppercase tracking-wider mb-2 block">Task Title</label>
                ...
                <div>
                  <label className="text-xs font-semibold text-[#8AA79B] uppercase tracking-wider mb-2 block">Detailed Instructions</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-semibold text-[#8AA79B] uppercase tracking-wider mb-2 block">Priority</label>
                  ...
                  <div>
                    <label className="text-xs font-semibold text-[#8AA79B] uppercase tracking-wider mb-2 block">Due Date</label>
                    <input type="date" className={inp} value={taskForm.dueDate} onChange={e => setTaskForm({ ...taskForm, dueDate: e.target.value })} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-semibold text-[#8AA79B] uppercase tracking-wider mb-2 block">Assign To Clerk</label>
                  ...
                  <div>
                    <label className="text-xs font-semibold text-[#8AA79B] uppercase tracking-wider mb-2 block">Link File (Optional)</label>
                    <div className="relative">
                      <div onClick={() => setIsFileDropdownOpen(!isFileDropdownOpen)}
                        className={`${inp} cursor-pointer flex justify-between items-center pl-10 ${isFileDropdownOpen ? "border-[#D4B65D] ring-4 ring-[#D4B65D]/10" : ""}`}>
                        ...
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#8AA79B]">📎</span>
                      </div>
                      {isFileDropdownOpen && (
                        <div className="absolute top-[calc(100%+8px)] left-0 right-0 bg-white border border-[#DDE5DD] rounded-2xl shadow-xl z-50 flex flex-col overflow-hidden max-h-72">
                          <div className="p-3 border-b border-[#F2F1E8] bg-[#FFFFFF]">
                            <div className="relative">
                              <input autoFocus type="text" placeholder="Search by file name…" value={fileSearch} onChange={e => setFileSearch(e.target.value)} onClick={e => e.stopPropagation()}
                                className="w-full bg-white border border-[#DDE5DD] p-2.5 pl-8 rounded-xl text-xs outline-none focus:ring-2 focus:ring-[#D4B65D]" />
                          ...
                          <div className="overflow-y-auto p-2 space-y-1" onClick={e => e.stopPropagation()}>
                            <button className={`w-full text-left px-4 py-2.5 rounded-xl text-xs font-medium hover:bg-[#FFFFFF] transition ${!taskForm.relatedFileId ? "bg-[#FBFAF6] text-[#27664D]" : "text-[#8AA79B]"}`}
                            ...
                            {myData.cases.filter(c => c.fileName.toLowerCase().includes(fileSearch.toLowerCase())).length > 0 && (
                              <div className="pt-2">
                                <p className="px-3 py-1 text-xs font-semibold text-[#8AA79B] uppercase tracking-wider">Court Cases</p>
                                {myData.cases.filter(c => c.fileName.toLowerCase().includes(fileSearch.toLowerCase())).map(c => (
                                  <button key={c.id} onClick={() => { setTaskForm({ ...taskForm, relatedFileId: c.id, relatedFileType: "case", relatedFileName: c.fileName }); setIsFileDropdownOpen(false); setFileSearch(""); }}
                                    className={`w-full text-left px-4 py-2.5 rounded-xl text-xs font-medium hover:bg-[#FFFFFF] transition truncate flex items-center gap-2 ${taskForm.relatedFileId === c.id ? "bg-[#FBFAF6] text-[#27664D]" : "text-[#27664D]"}`}>
                                    ⚖️ {c.fileName}
                                  </button>
                                ))}
                              </div>
                            ...
                            {myData.txs.filter(t => t.fileName.toLowerCase().includes(fileSearch.toLowerCase())).length > 0 && (
                              <div className="pt-2">
                                <p className="px-3 py-1 text-xs font-semibold text-[#8AA79B] uppercase tracking-wider">Transactions</p>
                                {myData.txs.filter(t => t.fileName.toLowerCase().includes(fileSearch.toLowerCase())).map(t => (
                                  <button key={t.id} onClick={() => { setTaskForm({ ...taskForm, relatedFileId: t.id, relatedFileType: "transaction", relatedFileName: t.fileName }); setIsFileDropdownOpen(false); setFileSearch(""); }}
                                    className={`w-full text-left px-4 py-2.5 rounded-xl text-xs font-medium hover:bg-[#FFFFFF] transition truncate flex items-center gap-2 ${taskForm.relatedFileId === t.id ? "bg-[#FBFAF6] text-[#27664D]" : "text-[#27664D]"}`}>
                                    💼 {t.fileName}
                                  </button>
                                ))}
                              </div>
                            ...
                            {myData.ltrs.filter(l => ((l as any).subject || (l as any).title || (l as any).fileName || "").toLowerCase().includes(fileSearch.toLowerCase())).length > 0 && (
                              <div className="pt-2 pb-2">
                                <p className="px-3 py-1 text-xs font-semibold text-[#8AA79B] uppercase tracking-wider">Letters</p>
                                {myData.ltrs.filter(l => ((l as any).subject || (l as any).title || (l as any).fileName || "").toLowerCase().includes(fileSearch.toLowerCase())).map((l: any) => {
                                  ...
                                  return (
                                    <button key={l.id} onClick={() => { setTaskForm({ ...taskForm, relatedFileId: l.id, relatedFileType: "letter", relatedFileName: lName }); setIsFileDropdownOpen(false); setFileSearch(""); }}
                                      className={`w-full text-left px-4 py-2.5 rounded-xl text-xs font-medium hover:bg-[#FFFFFF] transition truncate flex items-center gap-2 ${taskForm.relatedFileId === l.id ? "bg-[#FBFAF6] text-[#27664D]" : "text-[#27664D]"}`}>
                                      ✉️ {lName}
                                    </button>
                                  );
                                })}
              ...
              <div className="flex gap-4 mt-8 pt-6 border-t border-[#F2F1E8]">
                <button onClick={closeModal} className="flex-1 bg-white border border-[#DDE5DD] text-[#8AA79B] py-4 rounded-2xl text-xs font-semibold uppercase tracking-wider hover:bg-[#FFFFFF] transition">Cancel</button>
                <button onClick={handleSaveTask} className="flex-[2] bg-[#27664D] hover:bg-[#D4B65D] text-white py-4 rounded-2xl text-xs font-semibold uppercase tracking-wider shadow-lg transition flex items-center justify-center gap-2">
      ...
      {completingDraftId && (
        <div className="fixed inset-0 bg-[#17372C]/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[40px] p-10 shadow-2xl">
            <h3 className="text-xl font-bold text-[#17372C] mb-2">Complete Draft</h3>
            <p className="text-[#8AA79B] text-sm mb-7 leading-relaxed">Optionally attach the completed document before marking this as done.</p>
            <div className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-[#8AA79B] uppercase tracking-wider mb-2 block">Completion Note / Update (optional)</label>
                <textarea
                  className="w-full bg-[#FFFFFF] border border-[#DDE5DD] p-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#D4B65D]"
                  rows={3}
              ...
              <div>
                <label className="text-xs font-semibold text-[#8AA79B] uppercase tracking-wider mb-2 block">Upload Document (optional)</label>
                <input type="file" className="w-full bg-[#FFFFFF] border border-[#DDE5DD] p-3 rounded-xl text-sm outline-none" onChange={e => setCompleteForm({ ...completeForm, documentFile: e.target.files?.[0] || null })} />
              </div>
            </div>
            <div className="flex gap-4 mt-8">
              <button onClick={() => { setCompletingDraftId(null); setCompleteForm({ hoursSpent: "", documentFile: null, completionNote: "" }); }} className="flex-1 text-[#8AA79B] text-xs font-semibold uppercase">Cancel</button>
