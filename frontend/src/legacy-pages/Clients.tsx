...
const Clients: React.FC = () => {
  ...
  const handleDownloadReport = (client: any) => {
    ...
    const content = [
      ...
      `OUTSTANDING : KSh ${client.totalOwed.toLocaleString()}`,
      "",
      "LINKED MATTERS",
  ...
  const handleDownloadExpensesCSV = (client: any) => {
    ...
    lines.push(["Type", "File Name", "Billed (KSh)", "Paid (KSh)", "Balance (KSh)"]);
    ...
    lines.push(["Category", "Received (KSh)", "Spent (KSh)", "Net Balance (KSh)"]);
    ...
    if (otherExpenses.length > 0) {
      ...
      lines.push(["Date", "Purpose", "Amount (KSh)"]);
  ...
  const lbl = "block text-xs font-semibold text-[#8AA79B] uppercase tracking-widest mb-1.5";
  const inp = "w-full bg-[#FFFFFF] border border-[#F2F1E8] rounded-xl px-4 py-3 text-sm text-[#27664D] outline-none focus:ring-2 focus:ring-blue-400 transition";
  ...
  return (
    <div className="min-h-screen bg-[#F4F7F9] p-8">
      ...
      <div className="flex justify-between items-end mb-10">
        <div>
          <p className="text-xs font-semibold text-[#8AA79B] uppercase tracking-widest mb-1.5">
            FXJ Suits Kenya workspace
          </p>
          <h1 className="text-4xl font-bold text-[#17372C] leading-tight">
            Client Portfolio
          </h1>
        </div>
        <button
          onClick={() => setShowAddModal(true)}
          className="bg-[#17372C] text-white px-7 py-3.5 rounded-2xl text-sm font-semibold shadow-lg hover:bg-[#27664D] active:scale-95 transition-all"
        >
          + Register Client
        </button>
      </div>
      ...
      {/* SEARCH & SORT */}
      <div className="bg-white rounded-2xl shadow-sm border border-[#F2F1E8] mb-8 flex items-center gap-3 px-5">
        <span className="text-[#8AA79B] text-base select-none">🔍</span>
        <input
          type="text"
          placeholder="Search by name, company, or email..."
          className="flex-1 bg-transparent py-4 text-sm text-[#27664D] placeholder-slate-300 outline-none"
          value={searchTerm}
          onChange={e => setSearchTerm(e.target.value)}
        />
        {searchTerm && (
          <button onClick={() => setSearchTerm("")} className="text-[#8AA79B] hover:text-[#8AA79B] text-lg font-bold leading-none transition">x</button>
        )}
        <div className="w-px h-6 bg-[#FBFAF6]" />
        <select
          className="bg-transparent text-[#8AA79B] text-sm font-medium py-4 outline-none cursor-pointer"
          value={sortType}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {filteredClients.map(client => (
          <div
            ...
            className="bg-white rounded-3xl p-7 border border-[#F2F1E8] shadow-sm hover:shadow-xl hover:border-blue-200 transition-all cursor-pointer group"
          >
            <div className="flex justify-between items-start mb-5">
              <span className={`px-3 py-1 rounded-lg text-xs font-semibold ${client.type === "Corporate" ? "bg-purple-50 text-purple-600" : "bg-[#FBFAF6] text-[#27664D]"}`}>
                {client.type}
              </span>
              <span className="text-xs text-[#8AA79B]">#{client.id.split("-")[1]}</span>
            </div>
            <h3 className="text-xl font-semibold text-[#17372C] mb-1 group-hover:text-[#27664D] transition-colors leading-snug">
              {client.name}
            </h3>
            <p className="text-sm text-[#8AA79B] mb-6 truncate">{client.email || "No email on record"}</p>
            <div className="grid grid-cols-2 gap-3 border-t border-slate-50 pt-5">
              <div>
                <p className="text-xs font-semibold text-[#8AA79B] uppercase tracking-wider mb-0.5">Active Files</p>
                <p className="text-2xl font-bold text-[#17372C]">{client.totalFilesCount || 0}</p>
              </div>
              <div>
                <p className="text-xs font-semibold text-[#8AA79B] uppercase tracking-wider mb-0.5">Balance Due</p>
        ...
        {filteredClients.length === 0 && (
          <div className="col-span-3 py-24 text-center">
            ...
            <p className="text-sm text-[#8AA79B] font-medium">No clients match your search.</p>
          </div>
        )}
      </div>
      ...
      {selectedClient && (
        <div className="fixed inset-0 z-50 flex justify-end">
          <div className="absolute inset-0 bg-[#17372C]/40 backdrop-blur-sm" onClick={() => setSelectedClient(null)} />
          ...
          >
            <div className="bg-[#17372C] px-10 pt-10 pb-10 text-white">
            ...
            <div className="px-10 py-8">
              ...
              <div className="flex gap-2 p-1 bg-[#F2F1E8]/60 rounded-xl w-fit mb-8">
                <button
                  onClick={() => setActiveDrawerTab("Overview")}
                  className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeDrawerTab === "Overview" ? "bg-white text-[#17372C] shadow-sm" : "text-[#8AA79B] hover:text-[#27664D]"}`}
                >
                  Client Overview
                </button>
                {(currentUser?.role === 'accountant' || currentUser?.role === 'admin') && (
                  <button
                    ...
                    className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeDrawerTab === "Expenses" ? "bg-white text-[#17372C] shadow-sm" : "text-[#8AA79B] hover:text-[#27664D]"}`}
                  >
                    Expense Record
                  </button>
                )}
              ...
              {activeDrawerTab === "Overview" || !(currentUser?.role === 'accountant' || currentUser?.role === 'admin') ? (
                <>
                  <div className="grid grid-cols-3 gap-4 mb-8">
                    <div className="bg-[#FFFFFF] rounded-2xl p-4 text-center">
                      <p className="text-xs font-semibold text-[#8AA79B] uppercase tracking-wider mb-1">Total Files</p>
                      <p className="text-xl font-bold text-[#17372C]">{selectedClient.totalFilesCount}</p>
                    </div>
                  ...
                  <button
                    ...
                    className="w-full mb-8 py-3 rounded-xl border border-[#DDE5DD] text-sm font-semibold text-[#8AA79B] hover:bg-[#FFFFFF] transition-colors"
                  >
                    📥 Export Client Report
                  </button>

                  <div className="mb-8 bg-[#FBFAF6] rounded-2xl border border-blue-100 p-6">
                    <h4 className="text-xs font-semibold text-[#17372C] uppercase tracking-widest mb-4">
                      Internal Notes & Communication Log
                    </h4>
                    <textarea
                      className="w-full bg-white border border-blue-100 rounded-xl p-4 text-sm text-[#27664D] placeholder-slate-300 outline-none focus:ring-2 focus:ring-blue-400 resize-none mb-3"
                    ...
                    <button
                      ...
                      className="bg-[#17372C] text-white text-xs font-semibold px-5 py-2.5 rounded-xl hover:bg-[#27664D] transition-colors"
                    >
                      Save Log Entry
                    </button>
                      ...
                      {commLogs
                        ...
                        .map((log: any) => (
                          <div key={log.id} className="bg-white p-4 rounded-xl border border-blue-100">
                            <div className="flex justify-between text-xs font-semibold text-[#8AA79B] mb-1.5">
                              <span>{log.authorName || log.author}</span>
                              <span>{log.date}</span>
                            </div>
                            <p className="text-sm text-[#27664D] leading-relaxed">{log.note}</p>
                          </div>
                        ))}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                    <h4 className="text-xs font-semibold text-[#17372C] uppercase tracking-widest">
                      Linked Matter History
                    </h4>
                    <div className="flex gap-2 p-1 bg-[#FBFAF6] rounded-full">
                      <button
                        onClick={() => setActiveHistoryTab("Matters")}
                        className={`px-4 py-2 rounded-full text-xs font-semibold transition ${activeHistoryTab === "Matters" ? "bg-white text-[#17372C] shadow-sm" : "text-[#8AA79B] hover:text-[#27664D]"}`}
                      >
                        Matter History
                      </button>
                      <button
                        onClick={() => setActiveHistoryTab("Letters")}
                        className={`px-4 py-2 rounded-full text-xs font-semibold transition ${activeHistoryTab === "Letters" ? "bg-white text-[#17372C] shadow-sm" : "text-[#8AA79B] hover:text-[#27664D]"}`}
                      >
                        Letters
                      </button>
                    </div>
                      ...
                      <>
                        {selectedClient.cases.map((c: any) => (
                          <MatterRow key={c.id} title={c.fileName} badge="Court Case"
                            badgeColor="text-[#27664D] bg-[#FBFAF6]" sub={c.status}
                        ...
                        {selectedClient.totalFilesCount === 0 && (
                          <p className="text-[#8AA79B] text-sm italic py-6 text-center">No matters linked to this client.</p>
                        )}
                      </>
                    ) : (
                      <>
                        {selectedClientLetters.length > 0 ? (
                          ...
                          <p className="text-[#8AA79B] text-sm italic py-6 text-center">No letters linked to this client.</p>
                        )}
                      </>
                    )}
                  </div>
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        ...
                        <div className="mb-6 bg-gradient-to-r from-[#17372C] to-slate-800 rounded-2xl p-6 text-white shadow-lg">
                          <h5 className="text-xs font-semibold text-[#8AA79B] uppercase tracking-widest mb-4 flex items-center gap-2">
                            <span className="w-2 h-2 rounded-full bg-emerald-400" /> Legal Fees — Billable Revenue
                          </h5>
                          <div className="grid grid-cols-3 gap-6">
                            <div>
                              <p className="text-xs text-[#8AA79B] mb-2">Total Billed</p>
                              <p className="text-2xl font-bold text-blue-300">{fmt(fileBilled)}</p>
                            </div>
                            <div>
                              <p className="text-xs text-[#8AA79B] mb-2">Total Received</p>
                              <p className="text-2xl font-bold text-emerald-400">{fmt(totalLegalFees)}</p>
                            </div>
                            <div>
                              <p className="text-xs text-[#8AA79B] mb-2">Outstanding Balance</p>
                        ...
                        {disbursementCategories.length > 0 && (
                          <div className="mb-6 bg-gradient-to-r from-indigo-50 to-blue-50 border border-indigo-200 rounded-2xl p-6">
                            ...
                            <div className="space-y-2 mb-4">
                              {disbursementCategories.map((d, i) => (
                                <div key={i} className="flex items-center justify-between bg-white/80 rounded-xl p-3 border border-indigo-100">
                                  <span className="text-sm font-semibold text-[#27664D]">{d.name}</span>
                                  <div className="flex items-center gap-4 text-sm">
                                    ...
                                    <span className={`font-black px-2 py-0.5 rounded ${d.net > 0 ? 'bg-emerald-100 text-emerald-700' : d.net < 0 ? 'bg-red-100 text-red-600' : 'bg-[#FBFAF6] text-[#8AA79B]'}`}>
                            ...
                            <div className="flex items-center justify-between bg-indigo-100/50 rounded-xl p-3 border border-indigo-200">
                              ...
                              <span className={`text-lg font-black ${totalDisbursementNet > 0 ? 'text-emerald-600' : totalDisbursementNet < 0 ? 'text-red-600' : 'text-[#8AA79B]'}`}>
                        ...
                        <div className={`mb-8 rounded-2xl p-5 border ${overallBalance > 0 ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200'}`}>
                          <h5 className="text-xs font-semibold text-[#8AA79B] uppercase tracking-widest mb-3">Overall Account Position</h5>
                          <div className="grid grid-cols-3 gap-4">
                            <div>
                              <p className="text-xs text-[#8AA79B] mb-1">Total Money In</p>
                              <p className="text-lg font-bold text-emerald-600">{fmt(totalMoneyIn)}</p>
                            </div>
                            <div>
                              <p className="text-xs text-[#8AA79B] mb-1">Total Money Out</p>
                              <p className="text-lg font-bold text-red-500">{fmt(totalMoneyOut)}</p>
                            </div>
                            <div>
                              <p className="text-xs text-[#8AA79B] mb-1">Account Balance</p>
                          ...
                          {unmatchedExpenses > 0 && (
                            <p className="text-xs text-[#8AA79B] mt-3 italic">
                      ...
                      <>
                        ...
                        {uniqueFiles.length > 1 && (
                          <div className="mb-6 bg-gradient-to-r from-blue-50 to-blue-100 border border-blue-200 rounded-xl p-4">
                            <h5 className="text-xs font-semibold text-[#17372C] uppercase tracking-widest mb-3">Total Across All Files</h5>
                            <div className="grid grid-cols-4 gap-4">
                              <div>
                                <p className="text-xs text-[#8AA79B] mb-1">Total Billed</p>
                                <p className="text-lg font-bold text-[#17372C]">{fmt(totalBilled)}</p>
                              </div>
                              <div>
                                <p className="text-xs text-[#8AA79B] mb-1">Total Received</p>
                                <p className="text-lg font-bold text-emerald-600">{fmt(totalPaid)}</p>
                              </div>
                              <div>
                                <p className="text-xs text-[#8AA79B] mb-1">Total Spent</p>
                                <p className="text-lg font-bold text-orange-600">{fmt(totalSpent)}</p>
                              </div>
                              <div>
                                <p className="text-xs text-[#8AA79B] mb-1">Total Owed</p>
                        ...
                        {uniqueFiles.length > 0 ? (
                          <div className="space-y-4">
                            <h5 className="text-xs font-semibold text-[#17372C] uppercase tracking-widest">
                              {uniqueFiles.length > 1 ? 'Breakdown by File' : 'File Details'}
                            </h5>
                            {uniqueFiles.map((file: any) => (
                              <div key={file.name} className="bg-white border border-[#F2F1E8] rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow">
                                <p className="text-sm font-semibold text-[#17372C] mb-3 truncate">{file.name}</p>
                                <div className="grid grid-cols-4 gap-4">
                                  <div>
                                    <p className="text-xs text-[#8AA79B] mb-1">Billed</p>
                                    <p className="text-base font-bold text-[#17372C]">{fmt(file.billed)}</p>
                                  </div>
                                  <div>
                                    <p className="text-xs text-[#8AA79B] mb-1">Received</p>
                                    <p className="text-base font-bold text-emerald-600">{fmt(file.paid)}</p>
                                  </div>
                                  <div>
                                    <p className="text-xs text-[#8AA79B] mb-1">Spent</p>
                                    <p className="text-base font-bold text-orange-600">{fmt(file.spent || 0)}</p>
                                  </div>
                                  <div>
                                    <p className="text-xs text-[#8AA79B] mb-1">Balance</p>
                          ...
                          <p className="text-[#8AA79B] text-sm italic mb-4">No files or financial records for this client.</p>
                        )}

                        {/* Expense detail table if expenses exist */}
                        {selectedClient.expenses?.length > 0 && (
                          <>
                            <h5 className="text-xs font-semibold text-[#17372C] uppercase tracking-widest mt-8 mb-3">Expense Transactions</h5>
                            <div className="bg-white border border-[#F2F1E8] rounded-xl shadow-sm overflow-hidden">
                              <div className="overflow-x-auto">
                                <table className="w-full text-left border-collapse text-sm">
                                  <thead>
                                    <tr className="bg-[#FFFFFF] border-b border-[#F2F1E8] text-xs font-semibold text-[#8AA79B] uppercase tracking-widest">
                                  ...
                                  <tbody>
                                    {selectedClient.expenses.map((exp: any) => (
                                      <tr key={exp.id} className="border-b border-slate-50 last:border-0 hover:bg-[#FFFFFF]">
                                        <td className="p-3 whitespace-nowrap text-[#27664D]">{exp.date}</td>
                                        <td className="p-3">
                                          {exp.type === 'in'
                                        ...
                                        <td className="p-3 text-[#27664D] max-w-[150px] truncate" title={exp.relatedFileName}>{exp.relatedFileName || '-'}</td>
                                        <td className="p-3 text-[#27664D]">{exp.purpose || exp.description}</td>
                                        <td className={`p-3 text-right font-bold whitespace-nowrap ${exp.type === 'in' ? 'text-emerald-600' : 'text-red-500'}`}>
                                          {exp.type === 'in' ? '+' : '-'} KSh {Number(exp.amount).toLocaleString()}
                                        </td>
                                      </tr>
                                    ))}
                                  </tbody>
      ...
      {showEditModal && selectedClient && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-[#17372C]/60 backdrop-blur-sm" onClick={() => setShowEditModal(false)} />
          <div
          ...
          >
            <button
              ...
              className="absolute top-6 right-6 w-9 h-9 flex items-center justify-center rounded-full bg-[#FBFAF6] hover:bg-red-100 hover:text-red-500 text-[#8AA79B] font-bold transition-colors"
            >
              x
            </button>
            <div>
              <p className="text-xs font-semibold text-[#8AA79B] uppercase tracking-widest mb-1">Edit Client</p>
              <h2 className="text-2xl font-bold text-[#17372C]">Update Client Details</h2>
            </div>
            <div>
            ...
            <button
              ...
              className="w-full bg-[#17372C] text-white text-sm font-semibold py-4 rounded-2xl shadow-lg hover:bg-[#27664D] active:scale-95 transition-all disabled:opacity-40 disabled:cursor-not-allowed mt-2"
            >
              Save Changes
            </button>
          </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-[#17372C]/80 backdrop-blur-md" onClick={() => setShowAddModal(false)} />
          <form
          ...
          >
            ...
            <div>
              <p className="text-xs font-semibold text-[#8AA79B] uppercase tracking-widest mb-1">Client Intake</p>
              <h2 className="text-2xl font-bold text-[#17372C]">New Client Registration</h2>
            </div>
            <div>
              <label className={lbl}>Full Name / Company</label>
              <input required className={inp} placeholder="e.g. Nakato Sarah or Nakuru Holdings Ltd"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className={lbl}>Phone Number</label>
                <input className={inp} placeholder="+254 700 000000"
                  value={phone} onChange={e => setPhone(e.target.value)} />
              </div>
            </div>
            <div>
              <label className={lbl}>Email Address <span className="normal-case font-normal text-[#8AA79B]">(optional)</span></label>
              <input type="email" className={inp} placeholder="client@example.com"
                value={email} onChange={e => setEmail(e.target.value)} />
            </div>
            <button
              type="submit"
              className="w-full bg-[#17372C] text-white text-sm font-semibold py-4 rounded-2xl shadow-lg hover:bg-[#27664D] active:scale-95 transition-all mt-2"
            >
...
const MatterRow = ({ title, badge, badgeColor, sub, onOpen }: {
  ...
  <div className="flex justify-between items-center p-4 bg-[#FFFFFF] rounded-xl border border-[#F2F1E8] hover:border-blue-200 transition-colors">
    <div className="flex-1 min-w-0 mr-4">
      <p className="text-sm font-semibold text-[#17372C] truncate">{title}</p>
      <div className="flex items-center gap-2 mt-1">
        <span className={`text-xs font-semibold px-2 py-0.5 rounded-md ${badgeColor}`}>{badge}</span>
        <span className="text-xs text-[#8AA79B]">{sub}</span>
      </div>
    </div>
    <button
      onClick={e => { e.stopPropagation(); onOpen(); }}
      className="text-xs font-semibold text-[#D4B65D] hover:text-[#27664D] whitespace-nowrap transition-colors"
    >
      Open →
    </button>
  </div>
